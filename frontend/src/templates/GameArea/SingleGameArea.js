...
class SingleGameArea extends Component {
  constructor(props) {
    super(props);
  ...
  createGameElements = () => {
    ...
    pong.emit("createSingleGameElements", {
      w: width,
      h: height,
    });
    pong.on("singleGameElementsCreated", (data) => this.getGameElements(data));
    this.loop();
  };
  ...
  loop = () => {
    ...
    if (
      ...
      pong.emit("singleGameRequestSync");
    }
  ...
  handlePlayerMoveByKeyboard = (e) => {
    const { pong } = this.props;
    const key = e === "" ? "" : e.key;
    pong.emit("singleGameClientMove", key);
  };

  handlePlayerMoveByTouch = (move) => {
    const { pong } = this.props;
    pong.emit("singleGameClientMove", move);
  };
